...
MaxLikelihoodExecuteReplanner_t::MaxLikelihoodExecuteReplanner_t(ConnectedGraph_t &g, std::vector<int> currentPath)
{
	Timer t;
	...
	while (execute(g) and m_nReplan < 20)
	{
		m_nReplan++;
		std::cout << "\n#Replan: " << m_nReplan << "\n";
		...
		// }
		MaxLikelihoodSolver_t mlsolver1(g, m_start, m_goalSetD, m_targetPosesD, m_labelWeights);
		mlsolver1.MaxLikelihood_search(g);

		// check if the ground truth is solvable
		if (mlsolver1.getIsSolvable() == false)
		{
			m_isDoomed = true;
			break;
		}
		...
		{
			m_path = mlsolver1.getmPath();
			totalPlannedPathLength += (m_path.size() - 1);
		}

	}
	// You are reaching here either
	// (1) The problem is solved (execute(g) returns false, no need to replan)
	// (2) It turns out to be a doomed ground truth (m_isDoomed = true)
...
bool MaxLikelihoodExecuteReplanner_t::execute(ConnectedGraph_t &g)
{
...
bool MaxLikelihoodExecuteReplanner_t::updateMap(ConnectedGraph_t &g, std::vector<int> &labels)
{
	bool isSafe = true;
	// check each label
...
void MaxLikelihoodExecuteReplanner_t::updateLabelWeights(int label, bool mode)
{
	int temp_obs;
...
MaxLikelihoodExecuteReplanner_t::~MaxLikelihoodExecuteReplanner_t() {}